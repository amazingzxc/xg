...
class Data(object):
    def __init__(self,size,is_train,test_img_path=''):
        self.size=size
        ...
        numpy.random.shuffle(self.x_domain_data)
        numpy.random.shuffle(self.x_domain_data)

        self.x_domain_data_val = self.x_domain_data[:int(self._num_examples * 0.1)]
        self.y_domain_data_val = self.y_domain_data[:int(self._num_examples * 0.1)]

        self.x_domain_data = self.x_domain_data[int(self._num_examples * 0.1):]
        self.y_domain_data = self.y_domain_data[int(self._num_examples * 0.1):]

        self._num_examples -= int(self._num_examples * 0.1)
    ...
    def validation_data(self):
        return self.x_domain_data_val,self.y_domain_data_val
